...
class AnalyticsImplTest {
    ...
    @Test
    fun `logScreenView - logs screen_view event with screen name and class`() =
        runTest {
    ...
    @Test
    fun `logWorkerJob - logs worker_job_initiated event with interval and alerts count`() =
        runTest {
            val interval = 15L
            val alertsCount = 3L
    ...
    @Test
    fun `logWorkSuccess - logs worker_job_success event with success param`() =
        runTest {
    ...
    @Test
    fun `logWorkFailed - logs worker_job_failed event with success, method, and error code`() =
        runTest {
    ...
    @Test
    fun `logCityDetails - logs select_content event with item_id, item_name, and content_type`() =
        runTest {
            val cityId = 987L
    ...
    @Test
    fun `logAddServiceApiKey - when API key added successfully - logs event with success, method, and directed_from_error as false`() =
        runTest {
    ...
    @Test
    fun `logAddServiceApiKey - when API key not added - logs event with failure, method, and directed_from_error as true`() =
        runTest {
            val service = WeatherForecastService.WEATHER_API
